...
class GraphSpec extends Specification implements GraphData {
    ...
    def "Should extract proper values from dataset"() {
        ...
        List<Integer> data = graphCreator.extractSearchedColumnFirstDigits(getRows(correctInputNoHeader), 2)
        then: "Return all digits"
        data == [2, 4]
        when: "Entry contains only non-digit characters"
        data = graphCreator.extractSearchedColumnFirstDigits(getRows(inputWithSomeIncorrectValuesNoHeader), 2)
        then: "Row is saved as null"
        data == [null, 7]
        when: "Entry starts with zero"
        data = graphCreator.extractSearchedColumnFirstDigits(getRows(correctInputNoHeaderWithZeroEntry), 2)
